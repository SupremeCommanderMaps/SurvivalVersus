newInstance = function(ScenarioInfo, options, textPrinter, playerArmies)
    ...
    local unitCreator = import('/maps/final_rush_pro_5.v0015/src/survival/SurvivalUnitCreator.lua').newUnitCreator(
        ScenarioInfo,
        options,
        spawnOutEffect
    )

    local positions = import('/maps/final_rush_pro_5.v0015/src/survival/Positions.lua').newInstance()
    ...
    local function createSurvivalStructures()
        local survivalStructures = import('/maps/final_rush_pro_5.v0015/src/survival/SurvivalStructures.lua').newInstance()
        ...
        import('/maps/final_rush_pro_5.v0015/src/survival/IslandBases.lua').newInstance().spawn()
    end
    ...
    local setUp = function()
        ...
        if options.waterKillsAcu() then
            import('/maps/final_rush_pro_5.v0015/src/survival/CommanderWaterPain.lua')
                .newInstance(allUnits, textPrinter, isSurvivalUnit).runThread()

            import('/maps/final_rush_pro_5.v0015/src/artifacts/HillGuards.lua').newInstance().createHillGuards()
        end

        import('/maps/final_rush_pro_5.v0015/src/survival/ParagonEvent.lua').newInstance(
            ScenarioFramework,
            unitCreator,
            playerArmies,
            positions,
            import('/maps/final_rush_pro_5.v0015/src/lib/UnitCreationCallbacks.lua').newInstance(allUnits),
            textPrinter
        ).setUp()
    end
    ...
    local runBattle = function(textPrinter, playerArmies)
        if ScenarioInfo.Options.opt_FinalRushAggression == 1 then
            local agressionSpawner = import('/maps/final_rush_pro_5.v0015/src/survival/AggressionSpawner.lua').newInstance(
                StartingPlayersExistance,
                import('/maps/final_rush_pro_5.v0015/src/survival/RandomUnits.lua').newInstance(ScenarioInfo, ScenarioFramework),
                AttackLocations,
        ...
        local unitSpanwerFactory = import('/maps/final_rush_pro_5.v0015/src/survival/SurvivalSpawnerFactory.lua').newInstance(
            options,
            ScenarioFramework,
            unitCreator,
            playerArmies,
        ...
        local function getEventTextPrinter()
            return ScenarioInfo.Options.opt_FinalRushEventNotifications == 1 and textPrinter
                    or import('/maps/final_rush_pro_5.v0015/src/lib/NullTextPrinter.lua').newInstance()
        end

        local function runSurvivalRounds()
            local rounds = import('/maps/final_rush_pro_5.v0015/src/survival/SurvivalRounds.lua').newInstance(
                textPrinter,
                unitSpanwerFactory,
                options
            )
        ...
        local function runRandomEvents()
            if ScenarioInfo.Options.opt_FinalRushRandomEvents > 0 then
                local randomEvents = import('/maps/final_rush_pro_5.v0015/src/survival/RandomEvents.lua').newInstance(
                    ScenarioInfo,
                    getEventTextPrinter(),
                    allUnits,
                    isSurvivalUnit,
                    unitSpanwerFactory,
                    import('/maps/final_rush_pro_5.v0015/src/survival/BeetleEvent.lua').newInstance(unitCreator, spawnEffect)
                )
        ...
        local function runBountyHunters()
            if ScenarioInfo.Options.opt_FinalRushHunters > 0 then
                local hunters = import('/maps/final_rush_pro_5.v0015/src/survival/Hunters.lua').newInstance(
