newInstance = function(ScenarioInfo, options, textPrinter, playerArmies)
    ...
    local setUp = function()
        ...
        local survivalStructures = import('/maps/final_rush_pro_5.2.v0002/src/SurvivalStructures.lua').newInstance()

        if options.waterKillsAcu() then
            local commanderWaterPain = import('/maps/final_rush_pro_5.2.v0002/src/CommanderWaterPain.lua').newInstance(allUnits)
    ...
    local runBattle = function(textPrinter, playerArmies)
        local healthMultiplier = import('/maps/final_rush_pro_5.2.v0002/src/HealthMultiplier.lua').newInstance(
        ...
        if ScenarioInfo.Options.opt_FinalRushAggression == 1 then
            local agressionSpawner = import('/maps/final_rush_pro_5.2.v0002/src/AggressionSpawner.lua').newInstance(
                StartingPlayersExistance,
                import('/maps/final_rush_pro_5.2.v0002/src/RandomUnits.lua').newInstance(ScenarioInfo, ScenarioFramework),
                AttackLocations,
        ...
        local unitSpanwerFactory = import('/maps/final_rush_pro_5.2.v0002/src/SurvivalSpawnerFactory.lua').newInstance(
            options,
            ScenarioFramework,
            playerArmies,
            healthMultiplier,
        ...
        local rounds = import('/maps/final_rush_pro_5.2.v0002/src/SurvivalRounds.lua').newInstance(
            textPrinter,
            unitSpanwerFactory
        )
        ...
        local eventTextPrinter =
            ScenarioInfo.Options.opt_FinalRushEventNotifications == 1 and textPrinter
                or import('/maps/final_rush_pro_5.2.v0002/src/NullTextPrinter.lua').newInstance()

        if ScenarioInfo.Options.opt_FinalRushRandomEvents > 0 then
            local randomEvents = import('/maps/final_rush_pro_5.2.v0002/src/RandomEvents.lua').newInstance(
                ScenarioInfo,
                eventTextPrinter,
                allUnits,
                ListArmies,
        ...
        if ScenarioInfo.Options.opt_FinalRushHunters > 0 then
            local hunters = import('/maps/final_rush_pro_5.2.v0002/src/Hunters.lua').newInstance(
                eventTextPrinter,
                healthMultiplier,
                playerArmies,
