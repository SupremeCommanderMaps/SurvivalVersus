newInstance = function(ScenarioInfo)
    local options = import('/maps/final_rush_pro_5.v0013/src/frp/FinalRushOptions.lua').newInstance(ScenarioInfo.Options)
    ...
    local function setupTents()
        if ScenarioInfo.Options.opt_tents > 0 then
            local tents = import('/maps/final_rush_pro_5.v0013/src/artifacts/PrebuildTents.lua').newInstance(playerArmies);
    ...
    local function setupLighthouses()
        import('/maps/final_rush_pro_5.v0013/src/artifacts/CivilianLighthouses.lua').newInstance(textPrinter, playerArmies).spawn();
    end

    local function restrictTechs()
        if ScenarioInfo.Options.opt_timeunlocked ~= 0 then
            local techRestrictor = import('/maps/final_rush_pro_5.v0013/src/frp/TechRestrictor.lua').newInstance(
                buildRestrictor,
                textPrinter,
                playerArmies,
    ...
    local function setupParagonWars()
        if options.isParagonWars() then
            local paragonWars = import('/maps/final_rush_pro_5.v0013/src/paragon/ParagonWars.lua').newInstance(playerArmies, textPrinter)
            paragonWars.setUp()
        end
    end
    ...
    local function setupServival()
        if options.isSurvivalGame() then
            local survival = import('/maps/final_rush_pro_5.v0013/src/survival/Survival.lua').newInstance(ScenarioInfo, options, textPrinter, playerArmies)
            survival.start()
        end
    end
    ...
    local function setupAutoReclaim()
        if ScenarioInfo.Options.opt_AutoReclaim > 0 then
            ForkThread(
                import('/maps/final_rush_pro_5.v0013/src/frp/AutoReclaim.lua').AutoReclaimThread,
                options,
                playerArmies
            )
        end
    ...
    local function showWelcomeMessage()
        local welcomeMessages = import('/maps/final_rush_pro_5.v0013/src/frp/WelcomeMessages.lua').newInstance(ScenarioInfo, options, textPrinter)
