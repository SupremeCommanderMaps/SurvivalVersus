local modPath = 'FinalRushPro3'
local ScenarioUtils = import('/lua/sim/ScenarioUtilities.lua');
local ScenarioFramework = import('/lua/ScenarioFramework.lua');
local Utilities = import('/lua/utilities.lua');
local Entity = import('/lua/sim/Entity.lua').Entity;
local GameCommon = import('/lua/ui/game/gamecommon.lua');
local Aggression = import('/mods/'..modPath..'/lua/Aggression.lua');
-- Global Mod Check
...
enableT2 = function()
    getunlocktime = unlockwait()
    LOG("Waiting for T2 - ", getunlocktime)
    WaitSeconds(getunlocktime)
    local tblArmies = ListArmies() 
        for index, name in tblArmies do 
...
enableT3 = function()
	getunlocktime = unlockwait()	
    LOG("Waiting for t3 - ", getunlocktime * 2)
    WaitSeconds(getunlocktime * 2)
    local tblArmies = ListArmies() 
        for index, name in tblArmies do 
...
enableEXP = function()
	getunlocktime = unlockwait()	
    LOG("Waiting for t4 - ", getunlocktime * 3)
    WaitSeconds(getunlocktime * 3)
    local tblArmies = ListArmies() 
        for index, name in tblArmies do 
...
unlockwait = function()
	if (ScenarioInfo.Options.opt_timeunlocked == 1) then
		return 60 * 5
    end
	if (ScenarioInfo.Options.opt_timeunlocked == 2) then
		return 60 * 10
	end
	if (ScenarioInfo.Options.opt_timeunlocked == 3) then
		return 60 * 15
	end
	if (ScenarioInfo.Options.opt_timeunlocked == 4) then
		return 60 * 20
	end
end
